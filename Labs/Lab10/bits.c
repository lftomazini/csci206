...
int get_byte(int x, int n) {
  /* multiplies the n by 8, makes a binary and with 1's */
    n = (n << 3);
 ...
 */
int negate(int x) {
  /* every 0 must become 1 and vice-versa, after that 1 is added
     2's complement negation seen in class*/
    x = ~x;
    x++;
...
int is_positive(int x) {
    /* checks if number is negative or 0, then negates to return if positive*/
    return !((x & (1<<0xF)) | !x);
}
 ...
 */
int tmax(int n) {
  /* max number is 2^(n-1)-1, subtract 1 of exponent, calculate power of 2,
 ...
 */
int tmin(int n) {
  /* min value is 2^(n-1), subtracts one of exponent, calculate power of 2*/
    n--;
    n = (1 << n);
